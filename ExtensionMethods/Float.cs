...
public static partial class CheckValidatorsExtensions
{
    ...
    /// <param name="msg">Custom error message</param>
    /// <returns></returns>
    public static Check<float> IfBetweenOrEqual(this Check<float> data, float startValue, float endValue)
    {
        if (data.InvalidModel()) { return data; }
        if (data.Value >= startValue && data.Value <= endValue)
        {
            data.ThrowError($"The float '{data.Value}' is between or equal to '{startValue}' and '{endValue}'");
        }
        return data;
    }
}
